...
// the spend it funds. Never negative — an unspent surplus is discarded (v1 has
// no cross-period rollover).
...
// Per-category offset rolled up per bucket. Uncategorized spend (categoryId
// null) is never offset. The per-category max() is applied BEFORE summing so a
// surplus in one category can't subsidize overspend in another. Relies on the
// invariant that an expense's stored bucket equals its category's bucket.
