...
export class ProcessVoiceMessageUseCase {
    ...
    async execute(input: ProcessVoiceMessageInput): Promise<ProcessVoiceMessageOutput> {
        console.log(`Processing voice message from ${input.senderPhone}, media ID: ${input.mediaId}`);

        // Step 1: Download the audio file from WhatsApp
        const { buffer, metadata } = await this.mediaService.downloadMediaById(input.mediaId);
        console.log(`Downloaded audio: ${metadata.mime_type}, ${metadata.file_size} bytes`);

        // Step 2: Transcribe using Whisper
        const fileName = `audio_${input.mediaId}.${this.getFileExtension(metadata.mime_type)}`;
        const transcription = await this.transcriptionService.transcribe(buffer, fileName);

        console.log(`Transcription complete: "${transcription.text}" (${transcription.language})`);
        ...
        console.log(`Voice message processed successfully. Response: ${result.response}`);
