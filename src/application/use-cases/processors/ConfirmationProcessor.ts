...
export class ConfirmationProcessor implements MessageProcessor {
  ...
  canHandle(context: ProcessContext): boolean {
    ...
    return (
      context.textMessage.startsWith("confirm_delete_") ||
      context.textMessage.startsWith("cancel_delete_")
    );
  }

  async process(context: ProcessContext): Promise<ProcessOutput> {
    const action = context.textMessage.startsWith("confirm_delete_")
      ? "confirm"
      : "cancel";
    const transactionId = context.textMessage
      .replace("confirm_delete_", "")
      .replace("cancel_delete_", "");
